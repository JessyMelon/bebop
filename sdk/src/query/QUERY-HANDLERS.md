...
## CodeWiki lifecycle (SDK-only)

Handlers for CodeWiki namespace and multi-repo set initialization, selection, and status. There is no `gsd-tools.cjs` mirror yet; workflows should prefer these SDK queries and fall back to manual file/Git inspection only when unavailable. Golden policy treats these as explicit SDK-only exceptions in `sdk/src/golden/golden-policy.ts`; behavior coverage lives in `sdk/src/query/codewiki.test.ts` plus compiled-JS smoke tests.

| Dispatch | Purpose |
| -------- | ------- |
| `codewiki.init` / `codewiki init [--set <set-id>] [--repos <paths>] [--repo-id <id>]` | Creates missing `code-wiki/wiki-index.yaml`, repo manifests, starter `coder-llm-wiki/` directories, repo snapshots, and optional set manifests/snapshots. If `--set` is present and `--repos` is omitted, it discovers member repos from `codewiki.member_repos`, `sub_repos`, `planning.sub_repos`, `WORKSPACE.md`, then child Git repos. Member roles are inferred conservatively from repo/package names, dependencies, and framework markers. Idempotent: existing repo wiki files are reused; index and set manifest are merged when needed. This is a mutation command and is listed in `QUERY_MUTATION_COMMANDS`. |
| `codewiki.update` / `codewiki update [--set <set-id>] [--base <sha> --head <sha>] [--phase <N>] [--milestone <VERSION>] [--prepare-only\|--promote-only]` | Updates CodeWiki lifecycle metadata for selected repo manifests or optional set tuples. Explicit `--base/--head` wins for repo updates; `--phase` resolves the range from phase-scoped commit messages; `--milestone` resolves from the milestone tag or milestone-scoped commit messages; without those, the range is manifest commit to current `HEAD`. A repo with no matching phase/milestone commits is skipped rather than widened to `HEAD`. `--prepare-only` writes `coder-llm-wiki/00-meta/maintenance-plan.json` with changed files, classifications, repo targets, set targets, task items, seed policy, and evidence requirements without promoting manifests, indexes, tuples, or snapshots. `--promote-only` reuses existing maintenance plans for affected members only, requires maintenance verification to pass, then promotes repo manifests, repo index entries, optional set tuples, and source-diff-backed repo/set snapshots. Unchanged set members keep their existing tuple entries and do not need new plans for `--phase` / `--milestone` promotions. Without either flag, the handler preserves the legacy combined prepare+promote behavior for programmatic compatibility. DeepWiki and Repomix files are seed-only metadata and do not count as evidence. Mutation command. |
| `codewiki.freeze` / `codewiki freeze <version> [--set <set-id>] [--require-verified] [--allow-unverified]` | Freezes the selected repo namespace or multi-repo set for a shipped version. Writes freeze snapshots, marks repo/set manifests `status: frozen`, records `frozen_at` / `frozen_for_version`, and updates index status. When `--require-verified` is present, unresolved or invalid `codewiki.verify` results block freeze unless `--allow-unverified` is also present. Idempotent for repeated freezes of the same version. Mutation command. |
| `codewiki.project` / `codewiki project [--set <set-id>]` | Projects the selected repo namespace or multi-repo set into `.planning/codebase/codewiki-summary.md` for planner/executor consumption. This is a disposable projection; CodeWiki manifests, Git commits, and source evidence remain authoritative. Mutation command. |
| `codewiki.index` / `codewiki index [--set <set-id>]` | Indexes selected repo/set facts into `.planning/intel/codewiki.json` when `intel.enabled=true`, so `/gsd-intel query` can search CodeWiki repo IDs, set IDs, commit SHAs, wiki paths, snapshots, and open questions. Mutation command. |
| `codewiki.pack` / `codewiki pack [--set <set-id>] [--repo <repo-id>\|--repos <repo-id,repo-id>] [--style xml\|markdown\|json\|plain] [--force] [--dry-run]` | Runs Repomix for selected repo namespaces, writes `repomix-output.*` plus `repomix-output.meta.json` next to each repo manifest, and registers `paths.repomix_bundle` as seed-only context in the repo manifest. Mutation command. |
| `codewiki.deepwiki-export` / `codewiki deepwiki-export [--set <set-id>] [--repo <repo-id>\|--repos <repo-id,repo-id>] [--command <template>] [--register-existing] [--force] [--dry-run]` | Runs or registers DeepWiki exports for selected repo namespaces. The command template may use `{repo}`, `{repo_id}`, `{commit}`, `{branch}`, `{output_dir}`, `{output_md}`, and `{output_json}`. It writes `deepwiki-export/manifest.json` and registers DeepWiki paths as seed-only context. Mutation command. |
| `codewiki.contract` / `codewiki contract <name> --set <set-id> --producer <repo-id> --consumers <repo-id,repo-id>` | Creates or reuses a set-level `cross-repo/contracts/<name>.md` scaffold and registers it in `wiki-set.yaml` under `cross_repo.contracts` with `status: blocked`. Mutation command. |
| `codewiki.flow` / `codewiki flow <name> --set <set-id> --repos <repo-id,repo-id>` | Creates or reuses a set-level `cross-repo/flows/<name>.md` scaffold and registers it in `wiki-set.yaml` under `cross_repo.flows` with `status: blocked`. Mutation command. |
| `codewiki.verify` / `codewiki verify [--set <set-id>] [--phase <N>\|--milestone <VERSION>] [--maintenance-only]` | Verifies maintainer task completion after `codewiki.update` by reading `maintenance-plan.json`, `progress.json`, and `task-queue.json`. Completed tasks must have source evidence and updated files; blocked tasks must have reasons; unresolved, invalid, or blocked tasks fail the verification gate. Default mode also requires the selected repo or set to be fresh and baseline-complete, so init-only starter queues and unfinished bootstrap phases do not pass freeze/milestone gates. `--maintenance-only` is the pre-promotion gate for `codewiki.update`: it checks affected maintenance tasks without requiring manifests, set tuples, or baseline completion, and for phase/milestone set updates it verifies only members with matching update ranges. Read-only. |
| `codewiki.select` / `codewiki select [--set <set-id>]` | Select the repo CodeWiki namespace or multi-repo CodeWiki set matching the current checkout. Returns `current`, `dirty-current`, `stale`, `missing`, `frozen`, `set-current`, `set-partial`, or `set-stale` plus member commit details and the recommended next action. |
| `codewiki.status` / `codewiki status [--set <set-id>]` | Builds on `codewiki.select` with latest repo/set snapshots, open questions, status dashboard paths, and `.planning/codebase/` freshness hints. |

`codewiki.select`, `codewiki.status`, and `codewiki.verify` read `.planning/config.json` (`codewiki.root`, `codewiki.active_set`), `code-wiki/wiki-index.yaml`, repo `manifest.yaml`, set `wiki-set.yaml`, CodeWiki meta task files, and Git identity for member repos. They do not write files and are intentionally absent from `QUERY_MUTATION_COMMANDS`.

`codewiki.project` is the first consumer-facing projection hook. `/gsd-plan-phase` may call it when `codewiki.enabled=true` and `codewiki.projection.update_planning_codebase=true`.

`codewiki.index` is a derived intel hook. `/gsd-plan-phase` may call it when `codewiki.enabled=true`, `codewiki.projection.index_intel=true`, and `intel.enabled=true`.
