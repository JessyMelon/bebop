...
- 代码明确表达的内容写为事实
- 需要综合多处信息才能得到的内容，写为 inference 或放入 `开放问题`
- 无法确认设计意图时，直接写“不确定”
...
- routes/symbols 中的入口是否和 flow 的 `入口点` 一致
...
- 内容依赖已明确排除的源码范围
- 不能作为当前 durable wiki 事实继续推进
- 任务应在 `task-queue.json` 中标记为 `out-of-scope`，并保留 review 说明
